...
SELECT COUNT(*) 'Total' FROM ( # select the count (alias 'Total') from a inner query
SELECT c.c_custkey # select the customer keys
FROM CUSTOMER c # from the table customer (alias c)
LEFT JOIN ORDERS o # joined onto the orders table
...
) counts # required since inner query, has no use in this query
;